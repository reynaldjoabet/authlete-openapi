...
abstract class CIBAEndpoints extends BaseEndpoint {
  ...
  /**
    ...
    * backchannel authentication request further. /api/backchannel/authentication API
    */
  val backchannelAuthentication: Endpoint[
    ...
    /**
      ...
      * receives the result of end-user's decision about whether the end-user has approved or
      * rejected a client application's request on the authentication device.
  ...
  val backchannelAuthenticationComplete: Endpoint[
    ...
    /**
      * The API prepares JSON that contains an error. The JSON should be used as the response body
      * of the response which is returned to the client from the [backchannel authentication
  ...
  val backchannelAuthenticationFail: Endpoint[
    ...
    /**
      * This API prepares JSON that contains an `auth_req_id`. The JSON should be used as the
      * response body of the response which is returned to the client from the [backchannel
      * authentication
      * endpoint](https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html#auth_backchannel_endpoint)
