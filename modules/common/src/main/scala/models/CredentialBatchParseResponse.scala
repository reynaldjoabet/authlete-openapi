...
/**
  ...
  *
  * <p> The {@code action} value {@link Action#OK OK} means that the batch credential request is
  * valid. In this case, the implementation of the batch credential endpoint should call the
  * {@code /vci/batch/issue} API in order to issue verifiable credentials and/or transaction IDs and
  * generate a response to the request sender. </p>
  *
  * <hr> <h3>{@code action} = {@link Action#BAD_REQUEST BAD_REQUEST}</h3>
  *
  * <p> The {@code action} value {@link Action#BAD_REQUEST BAD_REQUEST} means that the batch
  * credential request is invalid. In this case, the implementation of the batch credential endpoint
  * should return an error response to the request sender. The HTTP status code and the content type
  * of the error response should be 400 and {@code application/json}, respectively. The value of the
  ...
  */
final case class CredentialBatchParseResponse(
    resultCode: String,
    resultMessage: String,
    action: Option[CredentialBatchParseResponse.CredentialBatchParseResponseAction],
    responseContent: Option[String],
    info: Option[Array[CredentialRequestInfo]]
) derives ConfiguredJsonValueCodec,
      Schema,
      Codec.AsObject

object CredentialBatchParseResponse {

  /**
    * The next action that the implementation of the batch credential endpoint should take.
    */
  enum CredentialBatchParseResponseAction(val value: String)
      derives ConfiguredJsonValueCodec,
        Schema,
        Codec.AsObject {
    ...
    /**
      ...
      */
    case OK extends CredentialBatchParseResponseAction("OK")

    /**
      * The batch credential request is invalid.
      */
    case BAD_REQUEST extends CredentialBatchParseResponseAction("BAD_REQUEST")

    /**
      * The batch credential request does not contain the access token or the access token is
      * invalid.
      */
    case UNAUTHORIZED extends CredentialBatchParseResponseAction("UNAUTHORIZED")

    /**
      * The feature of Verifiable Credentials is not enabled in the service configuration.
      */
    case FORBIDDEN extends CredentialBatchParseResponseAction("FORBIDDEN")

    /**
      * An error occurred on Authlete side.
      */
    case INTERNAL_SERVER_ERROR extends CredentialBatchParseResponseAction("INTERNAL_SERVER_ERROR")
