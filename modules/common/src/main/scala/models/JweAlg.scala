...
/**
  ...
  * to key transport scheme to be used by the client and by the server. For instance:
  *   - as `authorizationEncryptionAlg` value, it refers to the encoding algorithm used by server
  *     for transporting the keys on JARM objects
  *   - as `requestEncryptionAlg` value, it refers to the expected key transport encoding algorithm
  *     that server expects from the client when encrypting a Request Object
  *   - as `idTokenEncryptionAlg` value, it refers to the algorithm used by the server to key
  *     transport of id_tokens
  * **Please note that some of the algorithms are more secure than others, some are not supported
  * very well cross-platforms and some (like RSA1_5) are known to be weak**.
  */
enum JweAlg(val value: String) derives ConfiguredJsonValueCodec, Schema, Codec.AsObject {
...
object JweAlg {
  // implicit val codec: JsonValueCodec[JweAlg] =
