...
/** The signature algorithm for JWT. This value is represented on 'alg'
  * attribute of the header of JWT. Its semantics depend upon where it's
  * defined, such as:
  *   - as service accessTokenSignAlg value, it defines that access tokens are
  *     JWT and the algorithm used to sign it.
  *   - as client authorizationSignAlg value, it represents the signature
  *     algorithm used when creating a JARM response.
  *   - as client requestSignAlg value, it specifies the expected signature used
  *     by the client on a Request Object.
  */
enum JwsAlg(val value: String) derives Codec.AsObject {
