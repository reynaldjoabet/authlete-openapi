...
/** Represents an HSK (Hardware Security Key).
  ...
  * @param use
  *   The use of the key on the HSM. When the key use is "sig" (signature), the
  *   private key on the HSM is used to sign data, and the corresponding public
  *   key is used to verify the signature. When the key use is "enc"
  *   (encryption), the private key on the HSM is used to decrypt encrypted data
  *   which has been encrypted with the corresponding public key.
  * @param kid
