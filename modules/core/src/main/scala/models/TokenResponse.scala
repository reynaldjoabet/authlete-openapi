...
/** @param resultCode
  ...
  *   `password` and the token request is valid.
  * @param ticket
  *   The ticket which is necessary to call Authlete\'s `/auth/token/fail` API
  *   or `/auth/token/issue` API. This parameter has a value only if the value
  *   of `grant_type` request parameter is `password` and the token request is
  *   valid.
  * @param accessToken
  ...
  * @param subject
  *   The subject (= resource owner\'s ID) of the access token. Even if an
  *   access token has been issued by the call of `/api/auth/token` API, this
  *   parameter is `null` if the flow of the token request was [Client
  *   Credentials
...
object TokenResponse {
  enum TokenResponseAction(value: String) derives Codec.AsObject {
    case InternalServerError
        extends TokenResponseAction("INTERNAL_SERVER_ERROR")
