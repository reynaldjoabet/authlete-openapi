...
final case class DeviceFlowEndpoints() extends BaseEndpoint {
  ...
  /** This API parses request parameters of a device authorization request and
  ...
  /** This API returns information about what action the authorization server
    ...
    * request. /api/device/complete API
    */
  val deviceComplete =
    endpoint.post
...
case class DeviceAuthorizationApiRequest(
        deviceAuthorizationRequest: DeviceAuthorizationRequest
    )
case class DeviceCompleteApiRequest(
        deviceCompleteRequest: DeviceCompleteRequest
    )
case class DeviceVerificationApiRequest(
